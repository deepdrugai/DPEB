...
df_id_seq_ESM = pd.read_csv(file_path)
print("df_id_seq_ESM:" ,df_id_seq_ESM.head())

df_id_seq_ESM = df_id_seq_ESM.rename(columns={'ProteinID': 'protein_id'})
df_id_seq_ESM = df_id_seq_ESM.rename(columns={'ESM_Embeddings': 'Embeddings'})


# Filter df_id_seq_ESM to keep only proteins that are in the graph
df_id_seq_ESM_filtered = df_id_seq_ESM[df_id_seq_ESM["protein_id"].isin(all_proteins)]
#  Convert Embeddings from string to list
df_id_seq_ESM_filtered["Embeddings"] = df_id_seq_ESM_filtered["Embeddings"].apply(ast.literal_eval)

#  Create a tensor to store node features (initialize with zeros)
feature_dim = len(df_id_seq_ESM_filtered.iloc[0]["Embeddings"])  
node_features = torch.zeros((len(all_proteins), feature_dim), dtype=torch.float32)

#  Assign embeddings to correct node index
for _, row in df_id_seq_ESM_filtered.iterrows():
...
g.ndata["esm_features"] = node_features
g_alpha = g
...
save_path = "/data/saiful/ePPI/esm_ePPI_graph_with_map.pkl"
...
node_index = 22  
print(g.ndata["esm_features"][node_index])
...
node_features = g.ndata["esm_features"][node_indices]  # Assuming 'feat' is the feature key

...
print("Running models_name: ", models_name)
print("Physical GPU ID(s):", os.environ.get("CUDA_VISIBLE_DEVICES"))

sys.stdout = open(f"console_outputs/console_output_lr-3_full_graph_{concat_type}_concatenate_{models_name}_models.txt", "w")
# sys.stdout = open("console_outputs/console_output", "w")

print("Current Combination: ", concat_type)
print("Running models_name: ", models_name)
import dgl
...
from model import GAT, SAGE, GCN, GIN, RGCN, GTN, GTN_g2
EPOCHS = 20
...
def calculate_metrics(pos_score, neg_score, epoch, total_loss, it, desc="Training"):
    ...
    if desc == "Testing":
        ...
        print("Epoch: {:02d} | Final Performance: {:05d} | Loss: {:.4f} | {} Accuracy: {:.4f} | AUC Score: {:.4f}\n\n".format(epoch, it, total_loss / (it + 1), desc, running_testing_accuracy, auc))
        return
    elif desc == "Training":
        ...
        running_val_accuracy, auc = print_performance_metrics(flto, fltp, print_stuff = False)
        print("Epoch: {:02d} |  Iteration: {:05d} | Loss: {:.4f} | {} Accuracy: {:.4f} | AUC Score: {:.4f}".format(epoch, it, total_loss / (it + 1), desc, running_val_accuracy, auc))
        return running_val_accuracy, auc
...
def train(device, g, feature, edges, model, name):
    ...
    for epoch in range(EPOCHS):
        ...
        acc, auc = evaluate(model, g['val'], edges['val'], feature, epoch, device, "Validation")
        ...
        if acc > best_val_acc:
           ...
           # checkpoint_dir = "checkpoints" #"final_checkpoints"
           checkpoint_dir = "/data/saiful/ePPI/concatenated embeddings/checkpoints/"

           os.makedirs(checkpoint_dir, exist_ok=True)
           checkpoint_path = os.path.join(checkpoint_dir, f"{feature}_feat_{name}_lr_1e-3_{concat_type}_embedding_checkpoint.pth")
...
for key in g.ndata:
    ...
    print(f"  Min value: {emb.min().item()}")
    print(f"  Max value: {emb.max().item()}")
    print()
...
# import torch
# for key in ['protvec', 'biovec', 'alphafold', 'esm']:
#     g.ndata[key] = (g.ndata[key] - g.ndata[key].mean(dim=0)) / g.ndata[key].std(dim=0)
# g.ndata['concatenated'] = torch.cat([g.ndata[key] for key in ['protvec', 'biovec', 'alphafold', 'esm']], dim=1)


# Concatenate all embeddings along the feature dimension (dim=1) lr
...
print("g.ndata['concatenated'].shape:",g.ndata['concatenated'].shape)  # Should output: torch.Size([21435, 2988])

#------------------# load all four  embeddings ---------------#
print("After Concatenation g:", g)
# g = get_subgraph(g)
...
# flag 1.22 Normalize edge weights per node
# g.edata['weight'] = g.edata['weight'] / g.in_degrees(g.edges()[1])

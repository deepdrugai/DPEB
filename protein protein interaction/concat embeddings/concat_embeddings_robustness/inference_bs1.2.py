...
checkpoint_path = "/data/saiful/ePPI/concatenated embeddings/checkpoints selected/GIN/concatenated_feat_gin_lr_1e-4_Bio_Prot_embedding_checkpoint2.pth"
LR ="1e-4"


models_name = "GIN"  #  SAGE_GAT_GCN_GTN_GIN
N_iterations = 100
...
print("\nLoaded checkpoint file: ", checkpoint_name)


sys.stdout = open(f"./robustness_results/gin/console_output_lr{LR}_{concat_type}_concatenate_{models_name}_models_2.txt", "w")
# sys.stdout = open("console_outputs/console_output", "w")

print("Current Combination: ", concat_type)
print("Running models_name: ", models_name)
print("Loaded checkpoint file:", checkpoint_name)
...
def evaluate_robustness(model, graph, edges, feature, device, n_iterations=100, desc="Bootstrapping"):
    """
...
def load_trained_checkpoint(device, g, feature, edges, model, name):
    ...
    print("\n#---Bootstrapping Robustness Evaluation---#")
    evaluate_robustness(model, graph['test'], edges['test'], feature, device, n_iterations=N_iterations)  # 100
...
# Concatenate all embeddings along the feature dimension (dim=1)  gct

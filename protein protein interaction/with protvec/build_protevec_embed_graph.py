...
file_path = "/data/saiful/ePPI/stellar_df2_23k.csv"
df_id_seq_PV = pd.read_csv(file_path)
print("df_id_seq_PV:" ,df_id_seq_PV.head())

df_id_seq_PV = df_id_seq_PV.rename(columns={'UniProt ID': 'protein_id'})

# Filter df_id_seq_PV to keep only proteins that are in the graph
df_id_seq_PV_filtered = df_id_seq_PV[df_id_seq_PV["protein_id"].isin(all_proteins)]
#  Convert Embeddings from string to list
df_id_seq_PV_filtered["protvec_embedding"] = df_id_seq_PV_filtered["protvec_embedding"].apply(ast.literal_eval)

#  Create a tensor to store node features (initialize with zeros)
feature_dim = len(df_id_seq_PV_filtered.iloc[0]["protvec_embedding"])  
node_features = torch.zeros((len(all_proteins), feature_dim), dtype=torch.float32)

#  Assign embeddings to correct node index
for _, row in df_id_seq_PV_filtered.iterrows():
    protein = row["protein_id"]
    if protein in protein_to_idx:
        node_idx = protein_to_idx[protein]
        node_features[node_idx] = torch.tensor(row["protvec_embedding"], dtype=torch.float32)

#  Add node features to the DGL graph
g.ndata["protvec_feat"] = node_features
g_alpha = g
...
save_path = "/data/saiful/ePPI/protvec_ePPI_graph_with_map.pkl"
...
node_index = 22  
print(g.ndata["protvec_feat"][node_index])
...
node_features = g.ndata["protvec_feat"][node_indices]  # Assuming 'feat' is the feature key

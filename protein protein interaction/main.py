...
import sys
sys.stdout = open("console_outputs/console_output_lr-5_all_feats_alpha_GTN5.txt", "w")
...
from model import GAT, SAGE, GCN, GIN, RGCN, GTN, TAG, HGT, GGNN, GATv2, GTN4o, GTN_g2
EPOCHS = 10
...
def train(device, g, feature, edges, model, name):
    ...
    opt = th.optim.Adam(model.parameters(), lr=1e-5) #1e-2
...
def run_app(device, graph, edges):
  ...
  for feature in features:
    ...
    # # =============================================================================
    # # TAG doesnt work for block and  het case 
    # # =============================================================================
    
#     print(f"For Feature: {feature} ; flag 1.10i TAG Model")
    
#     tag = TAG(
#     in_size=in_size,
#     hid_size=[512, 256, 128],
#     out_dim=128,  # can be the same as last hid_size
#     k=3  # controls receptive field size
# )

#     tag_model = train(device, graph, feature, edges, tag, "tag")
#     print("=================================")
    ...
    # HGT
    # =============================================================================
    # print(f"For Feature: {feature} ; flag 1.10i HGT Model")


    #     # Add HGT model
    # hgt = HGT(
    #     in_size=in_size,
    #     hid_size=256,  # Match your other models' hidden sizes
    #     num_heads=4,   # Reasonable number of heads
    #     num_layers=3   # Match your 3-layer architecture
    # )

    # # Run HGT Model
    # print(f"For Feature: {feature} ; flag 1.10h HGT Model")
    # hgt_model = train(device, graph, feature, edges, hgt, "hgt")
    # print("=================================")
    ...
    # GGNN
    # =============================================================================
    # print(f"For Feature: {feature} ; flag 1.10i GGNN Model")
    #   # Extract the training graph from your graph dictionary.
    # train_graph = graph['train']
    # # Use the training graph to get the input feature dimension.
    # in_size = train_graph.ndata[feature].shape[1]

    # # --- Define other models if needed ---
    # # For example, instantiate your GAT, SAGE, etc.
    # # For GGNN, we'll show the corrected snippet:

    # print(f"For Feature: {feature} ; flag 1.10i GGNN Model")
    # # Convert the training graph (a DGLGraph) to a homogeneous graph
    # homo_graph = dgl.to_homogeneous(train_graph)
    
    # # Define GGNN model parameters
    # hidden_size = 256      # Hidden state dimension after message passing
    # num_layers = 3         # Number of GGNN layers (if you stack multiple)
    # num_timesteps = 3      # Number of message-passing steps within each GGNN layer

    # # Instantiate the GGNN model
    # ggnn = GGNN(in_size=in_size, hidden_size=hidden_size, num_layers=num_layers, num_timesteps=num_timesteps)
    # ggnn = ggnn.to(device)

    # print("For Feature: {0} ; Using GGNN Model".format(feature))
    # # Pass the homogeneous graph into your training function.
    # # The train() function already extracts the training graph using g["train"].
    # # Since we've converted train_graph separately for GGNN, we pass homo_graph.
    # ggnn_model = train(device, {'train': homo_graph, 'val': graph['val'], 'test': graph['test']},
    #                    feature, edges, ggnn, "ggnn")
    # print("=================================")
    ...
    #        out_dim= 256,
    #        num_heads= gat_num_heads)
    # print("For Feature: {0} ; flag 1.10b GATv2 Model".format(feature))
    # gatv2_model = train(device, graph, feature, edges, gatv2, "gatv2")
    # print("=================================")

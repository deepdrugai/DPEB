...
import sys
sys.stdout = open("console_outputs1.1/console_output_lr-4_full_graph_concat_embed_SAGE_GCN_GAT_main1.6.txt", "w")
...
#======== 3. Attention-Based Combination ======# 
print("#======== 3. Attention-Based Combination ======#")
class AttentionCombiner(nn.Module):
    def __init__(self, embed_dims, hidden_dim):
        super().__init__()
        self.projections = nn.ModuleList([
            nn.Linear(d, hidden_dim) for d in embed_dims
        ])
        self.attention = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),
            nn.Tanh(),
            nn.Linear(hidden_dim, len(embed_dims)),
            nn.Softmax(dim=-1)
        )
    
    def forward(self, embeddings):
        # Project all embeddings to same dimensionality
        projected = [proj(emb) for proj, emb in zip(self.projections, embeddings)]  # all [N, hidden_dim]
        stacked = torch.stack(projected, dim=1)  # Shape: [N, num_embeds, hidden_dim]

        # Compute attention over embeddings (shared attention)
        attn_weights = self.attention(stacked.mean(dim=1))  # [N, num_embeds]

        # Apply attention weights
        weighted = (attn_weights.unsqueeze(-1) * stacked).sum(dim=1)  # [N, hidden_dim]
        return weighted

# Define parameters
embed_dims = [300, 1024, 384, 1280]
hidden_dim = 512

# Initialize combiner
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
combiner = AttentionCombiner(embed_dims, hidden_dim).to(device)

# Combine embeddings
embeddings = [g.ndata['protvec'].to(device), g.ndata['biovec'].to(device), 
              g.ndata['alphafold'].to(device), g.ndata['esm'].to(device)]
g.ndata['concatenated'] = combiner(embeddings)

# Verify shape
print("Shape of attention features:", g.ndata['concatenated'].shape)  # Should be [21435, 512]

...
import sys
sys.stdout = open("console_outputs/console_output_lr-4_full_graph_concat_embed_allnorm_SAGE_GCN_GAT_3.7.txt", "w")
...
print('====== Combine embeddings in stages rather than all at once =====')
# First combine sequence-based embeddings
seq_combined = normalize(torch.cat([g.ndata['protvec'], g.ndata['biovec']], dim=1), p=2, dim=1)

# Then combine with structure-based
struct_combined = normalize(g.ndata['alphafold'], p=2, dim=1)
# Finally combine with ESM
g.ndata['concatenated'] = torch.cat([seq_combined, struct_combined, g.ndata['esm']], dim=1)

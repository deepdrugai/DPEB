...
import sys
sys.stdout = open("console_outputs1.1/console_output_lr-3_full_graph_concat_embed_SAGE_GCN_GAT_main1.7.txt", "w")
...
#======== 4. Multi-Head Attention Fusion ======# 
print("#======== 4. Multi-Head Attention Fusion ======#")
import torch
import torch.nn as nn

class MultiHeadFusion(nn.Module):
    def __init__(self, embed_dims, num_heads=4, out_dim=512):
        super().__init__()
        self.total_dim = sum(embed_dims)
        self.mha = nn.MultiheadAttention(embed_dim=self.total_dim, num_heads=num_heads)
        self.linear = nn.Linear(self.total_dim, out_dim)
    
    def forward(self, embeddings):
        concat_emb = torch.cat(embeddings, dim=-1).unsqueeze(0)  # [1, N, total_dim]
        attn_output, _ = self.mha(concat_emb, concat_emb, concat_emb)
        return self.linear(attn_output.squeeze(0))
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
fusion = MultiHeadFusion(embed_dims, num_heads=4, out_dim=out_dim).to(device)

# Combine embeddings
embeddings = [g.ndata['protvec'].to(device), g.ndata['biovec'].to(device), 
              g.ndata['alphafold'].to(device), g.ndata['esm'].to(device)]
g.ndata['concatenated'] = fusion(embeddings)

# Verify shape
print("Shape of fused features:", g.ndata['concatenated'].shape)  # Should be [21435, 512]
#======== 4. Multi-Head Attention Fusion ======# 

...
import sys
sys.stdout = open("console_outputs/console_output_lr-4_full_graph_concat_embed_allnorm_SAGE_GCN_GAT_3.3.txt", "w")
...
# Reduce each embedding to 128 dimensions first
print('====== Reduce each embedding to 128 dimensions first =====')
g.ndata['protvec_reduced'] = nn.Linear(300, 256)(g.ndata['protvec'])
g.ndata['biovec_reduced'] = nn.Linear(1024, 256)(g.ndata['biovec'])
g.ndata['alphafold_reduced'] = nn.Linear(384, 256)(g.ndata['alphafold'])
g.ndata['esm_reduced'] = nn.Linear(1280, 256)(g.ndata['esm'])

# Then concatenate
g.ndata['concatenated'] = torch.cat([
    g.ndata['protvec_reduced'],
    g.ndata['biovec_reduced'],
    g.ndata['alphafold_reduced'],
    g.ndata['esm_reduced']
], dim=1)

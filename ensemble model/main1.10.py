# 6. Nonlinear Transformations (MLP-Based)
# Imports
...
import sys
sys.stdout = open("console_outputs1.1/console_output_lr-3_full_graph_concat_embed_SAGE_GCN_GAT_main1.10.txt", "w")
...
#======== 6. Nonlinear Transformations (MLP-Based) ======# 
print("#======== 6. Nonlinear Transformations (MLP-Based) ======#")
import torch
import torch.nn as nn
...
class NonlinearProjector(nn.Module):
    ...
    def forward(self, embeddings):
        projected = [mlp(emb) for mlp, emb in zip(self.mlps, embeddings)]
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
projector = NonlinearProjector(embed_dims, proj_dim).to(device)
...
# Verify shape
print("Shape of nonlinear features:", g.ndata['concatenated'].shape)  # Should be [21435, 1024] (256 * 4)
#======== 6. Nonlinear Transformations (MLP-Based) ======# 

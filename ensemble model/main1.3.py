...
import sys
sys.stdout = open("console_outputs/console_output_lr-4_full_graph_concat_embed_allnorm_SAGE_GCN_GAT_3.6.txt", "w")
...
# Element-wise products between different embeddings
print('======  Use a GNN to refine the combined embeddings before link prediction =====')
...
class EmbeddingRefiner2(nn.Module):
...
class EmbeddingRefiner(nn.Module):
    def __init__(self, in_dim, hidden_dim):
        super().__init__()
        self.sage1 = dglnn.SAGEConv(in_dim, hidden_dim, aggregator_type='mean')
        self.sage2 = dglnn.SAGEConv(hidden_dim, hidden_dim, aggregator_type='mean')

    def forward(self, g, features):
        h = F.relu(self.sage1(g, features))
        h = self.sage2(g, h)
        return h

# Then refine with GNN
refiner = EmbeddingRefiner(2988, 512)
g = dgl.add_self_loop(g)

g.ndata['refined'] = refiner(g, g.ndata['concatenated'])
#------------------# load all four  embeddings ---------------#

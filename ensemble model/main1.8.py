# 8. Graph-Based Feature Enhancement (Diffusion)
...
import sys
sys.stdout = open("console_outputs1.1/console_output_lr-4_full_graph_concat_embed_SAGE_GCN_GAT_main1.8.txt", "w")
...
#======== 8. Graph-Based Feature Enhancement (Diffusion) ======# 
print("#======== 8. Graph-Based Feature Enhancement (Diffusion) ======#")
import torch
import torch.nn as nn
from dgl.nn import GraphConv
import torch.nn.functional as F

def graph_diffusion(g, features, num_layers=2, hidden_dim=512):
    # Project features to a manageable dimension if needed
    projector = nn.Linear(features.shape[1], hidden_dim).to(device)
    h = projector(features)
    
    # Apply graph convolution
    gcn = GraphConv(hidden_dim, hidden_dim).to(device)
    for _ in range(num_layers):
        h = F.relu(gcn(g, h))
    return h

# Concatenate embeddings
embeddings = [g.ndata['protvec'], g.ndata['biovec'], g.ndata['alphafold'], g.ndata['esm']]
concatenated = torch.cat(embeddings, dim=1).to(device)
g = dgl.add_self_loop(g)
# Apply diffusion
g.ndata['concatenated'] = graph_diffusion(g, concatenated, num_layers=3, hidden_dim=512)

# Verify shape
print("Shape of diffused features:", g.ndata['concatenated'].shape)  # Should be [21435, 512]
#======== 8. Graph-Based Feature Enhancement (Diffusion) ======# 

# 5. Embedding-Specific Transformations (Projection Layers
# Imports
...
import sys
sys.stdout = open("console_outputs1.1/console_output_lr-3_full_graph_concat_embed_SAGE_GCN_GAT_main1.9.txt", "w")
...
#======== 5. Embedding-Specific Transformations (Projection Layers ======# 
print("#======== 5. Embedding-Specific Transformations (Projection Layers ======#")
import torch
import torch.nn as nn

class EmbeddingProjector(nn.Module):
    def __init__(self, embed_dims, proj_dim=256):
        super().__init__()
        self.projectors = nn.ModuleList([nn.Linear(dim, proj_dim) for dim in embed_dims])
    
    def forward(self, embeddings):
        projected = [proj(emb) for proj, emb in zip(self.projectors, embeddings)]
        return torch.cat(projected, dim=-1)

# Define parameters
embed_dims = [300, 1024, 384, 1280]
proj_dim = 256  # Each embedding is projected to 256 dimensions

# Initialize projector
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
projector = EmbeddingProjector(embed_dims, proj_dim).to(device)

# Combine embeddings
embeddings = [g.ndata['protvec'].to(device), g.ndata['biovec'].to(device), 
              g.ndata['alphafold'].to(device), g.ndata['esm'].to(device)]
g.ndata['concatenated'] = projector(embeddings)
